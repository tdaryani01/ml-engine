...
class LRScheduler(ABC):
    ...
    @abstractmethod
    def step(self, epoch):
        """Computes and returns the decayed learning rate for the active epoch loop[cite: 6]."""
        pass
...
class StepDecay(LRScheduler):
    ...
    def step(self, epoch):
        """Computes step-decayed learning rate based on elapsed epochs[cite: 6]."""
        self.current_lr = self.initial_lr * (self.drop_ratio ** np.floor(epoch / self.epochs_per_drop))
        return self.current_lr

class ExponentialDecay(LRScheduler):
    """Decays the learning rate exponentially over each epoch[cite: 6]."""
    def __init__(self, initial_lr, decay_rate=0.96):
        super().__init__(initial_lr)
        self.decay_rate = decay_rate

    def step(self, epoch):
        """Computes exponentially decayed learning rate for the given epoch[cite: 6]."""
