...
def stream_shards_to_queue(shard_dir: str, queue_name: str, amqp_url: str) -> None:
    """
    Scans directory for .npy shards, serializes matrices to raw bytes,
    and publishes them directly into the AMQP broker queue.
    """
    ...
    logging.info(f"Target egress channel locked. Sending {len(shard_files)} shards to queue: '{queue_name}'")
    ...
    try:
        for idx, filename in enumerate(shard_files):
            ...
            properties = pika.BasicProperties(
                ...
                headers={"shape": shape_string, "shard_index": idx}
            )
            ...
            logging.info(f"[Egress Stream] Successfully pushed {filename} | Shape: ({shape_string})")
