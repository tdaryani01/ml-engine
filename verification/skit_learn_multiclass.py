...
def run_external_mlp_benchmark():
    ...
    print("==================================================")
    print("   SKLEARN PRODUCTION MLP CLASSIFIER BENCHMARK   ")
    print("==================================================")
    ...
    # Ingest categorical targets (0.0, 1.0, 2.0) mapped straight to integer footprints
    ...
    # Instantiate Scikit-Learn's multi-class engine mapping your parameters exactly
    external_mlp = MLPClassifier(
        hidden_layer_sizes=hidden_layers,
        activation='relu',         # Matches your custom activation hidden layers
        solver='adam',             # Matches your patched Adam state tracking optimization pass
        alpha=alpha_l2,
    ...
    print("\nTraining Scikit-Learn backbone classifier...")
    external_mlp.fit(X_train, y_train)

    # Predict categorical integers for accuracy tracking
    train_preds = external_mlp.predict(X_train)
    val_preds = external_mlp.predict(X_val)

    # Predict Softmax probability matrices for Cross-Entropy Loss computation
    train_probs = external_mlp.predict_proba(X_train)
    val_probs = external_mlp.predict_proba(X_val)

    print("\n==================================================")
    print("         EXTERNAL ARCHITECTURE VERIFICATION       ")
    print("==================================================")
    print(f"Sklearn Train Loss (Cross-Entropy): {log_loss(y_train, train_probs):.6f}")
    print(f"Sklearn Val Loss (Cross-Entropy)  : {log_loss(y_val, val_probs):.6f}")
    print(f"Sklearn Train Accuracy            : {accuracy_score(y_train, train_preds) * 100:.2f}%")
    print(f"Sklearn Validation Accuracy       : {accuracy_score(y_val, val_preds) * 100:.2f}%")
    print("==================================================")
